...
public:
	...
	BYTE GetMemberChannel(DWORD dwID);
	long GetMemberMapIndex(DWORD dwID);
...
struct FPartyDropDiceRoll
{
	...
	void Process(const LPCHARACTER mobVictim)
	{
		if ((!mobVictim || (mobVictim->GetMobRank() >= MOB_RANK_BOSS && mobVictim->GetMobRank() <= MOB_RANK_KING)) && m_itemOwner->GetParty() && m_itemOwner->GetParty()->GetNearMemberCount() > 1)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, LC_TEXT("@*** Rolling for the following item: %s ***"), m_itemDrop->GetLocaleName());
	...
	void operator () (LPCHARACTER ch)
	{
		...
		while (true)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, LC_TEXT("@*** ->%s - Dice score: %4d ***"), ch->GetName(), pickedNumber);
			break;
		}
	}
};
