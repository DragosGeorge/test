...
void CParty::ChatPacketToAllMember(BYTE type, const char* format, ...)
{
	...
	LPCHARACTER ch = GetLeaderCharacter();
	...
	if (type != CHAT_TYPE_COMMAND)
		localeFormat = FindLocaleString(format, d->GetLanguage());
	else
		localeFormat = format;
...
void CParty::SendPartyRemoveOneToAll(DWORD pid)
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		{
			//if (it->second.pCharacter->IsWarping())
			//	continue;
...
void CParty::SendPartyJoinAllToOne(LPCHARACTER ch)
{
	...
	for (TMemberMap::iterator it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		//BroadcastRestrictedNotice(LC_TEXT("aici e problema"));
		p.pid = it->first;
...
#if defined(__WJ_SHOW_PARTY_ON_MINIMAP__)
long CParty::GetMemberMapIndex(DWORD dwPID)
{
	if (!dwPID)
		return 0;
...
BYTE CParty::GetMemberChannel(DWORD dwPID)
{
	if (!dwPID)
		return 0;
