...
void CHARACTER::PetAttrChange(uint8_t bPetSlotIndex, uint8_t bMaterialSlotIndex)
{
	...
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your pet's stats have been modified."));
}
#endif
...
namespace PetEvolveItem
{
	...
	{
		if (evolveStage == 1)
			return { {55003, 10}, {30058, 10}, {30073, 10}, {30041, 10}, {30017, 5}, {30074, 5}, {30088, 5}, };
		else if (evolveStage == 2)
			return { {55004, 10}, {27994, 2}, {30035, 10}, {30089, 10}, {30031, 10}, {30011, 5}, {30080, 5} };
		else if (evolveStage == 3)
			return { {55005, 10}, {30083, 2}, {27992, 2}, {27993, 2}, {30086, 10}, {30077, 10}, {30550, 5} };
...
bool CGrowthPetSystemActor::ItemCubeFeed(uint8_t bIndex, const uint16_t iFeedItemsCubeSlot[9], uint16_t wFeedItemsCount)
{
	...
	{
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot use any soulbound items."));
		return false;
	}
#endif
	...
	{
		uint32_t bPetEvolution = GetEvolution();

		if (bPetEvolution == PET_MAX_EVOLVE) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your pet has already reached the highest stage of evolution."));
			return false;
		}
		...
		size_t itemlistcnt = EvItems.size();
		...
		{
			int slotIndex = iFeedItemsCubeSlot[i];
			LPITEM item = GetOwner()->GetInventoryItem(slotIndex);
		...
		{
			int fullslot = 0;
			BYTE playerLocale = GetOwner()->GetLanguage();
			for (const auto& v : EvItems)
			{
				uint32_t itemVnum = v.first;
				uint8_t itemCount = v.second;

				const char* itemLocaleName = LC_ITEM_NAME(itemVnum, playerLocale);

				int itemOwnedCount = GetOwner()->CountSpecifyItem(itemVnum);
				if (itemOwnedCount < itemCount) {
					GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s is insufficient. You need %d for the next stage of evolution."), itemLocaleName, itemCount - itemOwnedCount);
				}
				else
				{
					++fullslot;
		...
		if (iItemFound == itemlistcnt)
		{
			for (int i = 0; i < PET_MAX_FEED_SLOT; ++i)
			{
				...
				if (GetOwner()->GetInventoryItem(slotIndex)) {
					uint32_t itemVNum = GetOwner()->GetInventoryItem(slotIndex)->GetVnum();
					GetOwner()->RemoveSpecifyItem(itemVNum, EvItems[itemVNum]);
				}
			}

			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You've taught your pet well! It's quickly learning its new skills!"));
			EvolvePet();
		}
	...
	if (bIndex == PET_EXP_ITEM_WINDOW)
	{
		...
		else if (pFeedItem->GetType() == ITEM_WEAPON || pFeedItem->GetType() == ITEM_ARMOR)
		{
			...
			{
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s refuses to eat anything!"), m_pkGrowthPet->GetName());
				return false;
			}
		}
	}
...
bool CGrowthPetSystemActor::LearnPetSkill(uint8_t bSkillBookSlotIndex, uint8_t bSkillBookInvenIndex)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your pet has already learnt this skill and refuses to practice it again."));
			return false;
		}
	}
...
bool CGrowthPetSystemActor::PetSkillUpgradeRequest(uint8_t bSkillSlot, uint8_t bSkillIndex)
{
	...
	{
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You do not have enough Yang."));
		return false;
	}
...
bool CGrowthPetSystemActor::EvolvePet()
{
	...
	else
	{
		...
		{
			LC_TEXT("the level Young"),
			LC_TEXT("the level Wild"),
			LC_TEXT("the level Valiant"),
			LC_TEXT("the level Heroic")
		};

		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has evolved to %s! Excellent!"),
			m_pkPetSeal->GetName(), sEvolveName[GetEvolution()]);

		if (GetEvolution() == 2)
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Acquire the Valiant Pet Book for your pet's hero training."));
	}
...
bool CGrowthPetSystemActor::Determine()
{
	...
	{
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("To determine a pet's destiny you need a Pet Destiny Orb."));
		return false;
	}
	...
	{
		m_pkOwner->RemoveSpecifyItem(55032, 1);
		m_pkOwner->GetDesc()->Packet(&sPacket, sizeof(TPacketGCGrowthPetAttrDetermineResult));
		return true;
	}
...
void CHARACTER::RevivePet(const TPacketCGGrowthPetReviveRequest* revivePacket, LPITEM pSummonItem)
{
	for (int i = 0; i < PET_REVIVE_MATERIAL_SLOT_MAX; ++i)
	{
		...
		if (material->GetType() == ITEM_PET && material->GetSubType() != PET_PRIMIUM_FEEDSTUFF)
			return;
...
void CGrowthPetSystemActor::SetPetLevel(uint8_t bLevel)
{
	...
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has reached level %d!"), m_pkPetSeal->GetName(), GetPetLevel());
}
...
bool CGrowthPetSystemActor::SetExp(uint32_t dwExpValue, uint8_t bExpMode)
{
	...
	if (m_PetInfo.exp_monster >= m_PetInfo.next_exp_monster && m_PetInfo.exp_item < m_PetInfo.next_exp_item)
	{
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your pet has gathered enough experience. It now requires Item EXP to reach the next stage of evolution."));
	}
	...
	if (m_PetInfo.exp_monster >= m_PetInfo.next_exp_monster && m_PetInfo.exp_item >= m_PetInfo.next_exp_item)
	{
		...
		if (CanIncreasePetLevel())
			...
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your pet is ready to evolve. Use Pet Books to achieve this."));
	}
...
bool CGrowthPetSystemActor::Dismiss()
{
	...
	if (m_pkOwner)
	{
		...
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have dismissed %s."), m_PetInfo.pet_nick);
	}
...
uint32_t CGrowthPetSystemActor::Summon(const char* petName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	const DWORD dwPetVnum = pSummonItem->GetValue(0);
#else
	const DWORD dwPetVnum = GetRaceVnum();
#endif
	m_pkGrowthPet = CHARACTER_MANAGER::Instance().SpawnMob(dwPetVnum, m_pkOwner->GetMapIndex(), x, y, m_pkOwner->GetZ(), false, iRotation, false);
	if (!m_pkGrowthPet)
	{
	...
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have summoned %s."), m_PetInfo.pet_nick);
...
void CGrowthPetSystemActor::GiveBuff()
{
	...
	m_pkOwner->AddAffect(AFFECT_GROWTH_PET, aApplyInfo[APPLY_MAX_HP].bPointType, hp_value, 0, expire_time, 0, false);
	m_pkOwner->AddAffect(AFFECT_GROWTH_PET, aApplyInfo[APPLY_MAX_SP].bPointType, sp_value, 0, expire_time, 0, false);
	m_pkOwner->AddAffect(AFFECT_GROWTH_PET, aApplyInfo[APPLY_DEF_GRADE_BONUS].bPointType, def_value, 0, expire_time, 0, false);
