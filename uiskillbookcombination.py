...
class SkillBookCombinationWindow(ui.ScriptWindow):
	...
	def Close(self):
		...
		del self.SkillBookButton
		del self.ChestButton
	...
	def Open(self):
		...
		try:
			...
			## Button Event
			self.ChestButton = self.GetChild("chest_button")
			self.SkillBookButton = self.GetChild("skillbook_button")
			#self.CancelButton = self.GetChild("cancel_button")
			self.SkillBookButton.SetEvent(ui.__mem_func__(self.ClickSkillBookButton))
			self.ChestButton.SetEvent(ui.__mem_func__(self.ClickChestButton))
	...
	def SelectEmptySlot(self, slotIndex):
		...
		if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
			if player.SLOT_TYPE_INVENTORY != attachedSlotType and\
				...
				player.SLOT_TYPE_ATTRIBUTE_INVENTORY != attachedSlotType:
				return
			...
			if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
				if self.inven or self.invenSpecial:
					...
					if self.invenSpecial.GetInventoryType() == 4:
						if (item.ATTRIBUTE_INVENTORY_SLOT_START + (special_page * SPECIAL_INVENTORY_PAGE_SIZE)) <= idx < (item.ATTRIBUTE_INVENTORY_SLOT_START + (((special_page + 1) * SPECIAL_INVENTORY_PAGE_SIZE))):
							idx = idx - (item.ATTRIBUTE_INVENTORY_SLOT_START + (special_page * SPECIAL_INVENTORY_PAGE_SIZE))
							self.invenSpecial.wndItem.SetCanMouseEventSlot(idx)
			else:
	...
	def ClickChestButton(self):
		...
		net.SendSkillBookCombinationForChestPacket(self.SkillBookList, SKILLBOOK_COMBI_START)
		...
		for index, key in self.SkillBookList.items():
			if not isinstance(key, types.NoneType):
				...
				if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
					...
					if self.invenSpecial.GetInventoryType() == 4:
						if (item.ATTRIBUTE_INVENTORY_SLOT_START + (special_page * SPECIAL_INVENTORY_PAGE_SIZE)) <= i < (item.ATTRIBUTE_INVENTORY_SLOT_START + (((special_page + 1) * SPECIAL_INVENTORY_PAGE_SIZE))):
							lock_idx = i - (item.ATTRIBUTE_INVENTORY_SLOT_START + (special_page * SPECIAL_INVENTORY_PAGE_SIZE))
							self.invenSpecial.wndItem.SetCantMouseEventSlot(lock_idx)
			else:
				continue
