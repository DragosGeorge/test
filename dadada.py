...
class ToolTip(ui.ThinBoard):
	...
	OBIECT_COLOR = 0xffFFFF00
	COLOR_VNUM = 0xffFFFF00
	TOOLTIP_SPECIAL_COLOR = grp.GenerateColor(1.0,1.0,0.5,1.0)
...
class ItemToolTip(ToolTip):
	...
	def AppendChestDropInfo(self, itemVnum):
		...
		if hasinfo:
			self.AppendSpace(5)
			self.AppendTextLine(localeInfo.TOOLTIP_INFO_CHEST_DROP, self.TOOLTIP_SPECIAL_COLOR)
	...
	def SetShopItem(self, slotIndex, isPrivateShop = False):
		...
		if app.ENABLE_MULTISHOP:
			if shop.GetBuyWithItem(slotIndex) != 0:
				...
				self.AppendPrice(price, isPrivateShop)
		else:
	...
	def AddItemData_Offline(self, itemVnum, itemDesc, itemSummary, metinSlot, attrSlot):
		...
		if chr.IsGameMaster(player.GetMainCharacterIndex()):
			self.AppendTextLine("(VNUM: %d)" % (int(itemVnum)), self.OBIECT_COLOR)
...
#					
#					if not caracter:
#						chat.AppendChat(7,"nu e niciun caracter")
#							
...
#						self.__ModelPreview(itemVnum, 3, player.GetRace())
				if isCostumeWeapon:
					...
					antiflags = {
						item.ITEM_ANTIFLAG_WARRIOR: {0: 4, 1: 0},
						item.ITEM_ANTIFLAG_ASSASSIN: {0: 1, 1: 5},
						item.ITEM_ANTIFLAG_SHAMAN: {0: 3, 1: 7}
					}
					...
					for antiflag, sex_mapping in antiflags.items():
						if not item.IsAntiFlag(antiflag):
							if sex in sex_mapping:
								self.__ModelPreview(itemVnum, 3, sex_mapping[sex])

					if not item.IsAntiFlag(item.ITEM_ANTIFLAG_SURA):
						if race in sura_race_mapping:
							if sex in sura_race_mapping[race]:
								self.__ModelPreview(itemVnum, 3, sura_race_mapping[race][sex])

				elif isCostumeBody:
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_WARRIOR) == False and (player.GetRace() == 4 or player.GetRace() == 0):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 4)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 0)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_ASSASSIN) == False and (player.GetRace() == 5 or player.GetRace() == 1):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 1)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 5)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_SURA) == False and (player.GetRace() == 6 or player.GetRace() == 2):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 6)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 2)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_SHAMAN) == False and (player.GetRace() == 3 or player.GetRace() == 7):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 3)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 7)

					warrior_costume = [] #Adauga vnum-ul la costum in [] (exemplu [41003,41004])
					assassin_costume = [] #Adauga vnum-ul la costum in [] (exemplu [41003,41004])
					sura_costume = [54310]  # Adauga vnum-ul la costum in [] (exemplu [41003,41004])
					shaman_costume = []  # Adauga vnum-ul la costum in [] (exemplu [41003,41004])

					if itemVnum in warrior_costume:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 1)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 5)

					if itemVnum in assassin_costume:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 4)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 0)

					if itemVnum in sura_costume:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 6)
					...
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_WARRIOR) == False and (player.GetRace() == 4 or player.GetRace() == 0):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(item.GetValue(3), 1, 4)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(item.GetValue(3), 1, 0)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_ASSASSIN) == False and (player.GetRace() == 5 or player.GetRace() == 1):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(item.GetValue(3), 1, 1)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(item.GetValue(3), 1, 5)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_SURA) == False and (player.GetRace() == 6 or player.GetRace() == 2):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(item.GetValue(3), 1, 6)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(item.GetValue(3), 1, 2)
					if item.IsAntiFlag(item.ITEM_ANTIFLAG_SHAMAN) == False and (player.GetRace() == 3 or player.GetRace() == 7):
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(item.GetValue(3), 1, 3)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(item.GetValue(3), 1, 7)

					warrior_hair = []  # Adauga vnum-ul la freza in [] (exemplu [41003,41004])
					assassin_hair = []  # Adauga vnum-ul la freza in [] (exemplu [41003,41004])
					sura_hair = []  # Adauga vnum-ul la freza in [] (exemplu [41003,41004])
					shaman_hair = []  # Adauga vnum-ul la freza in [] (exemplu [41003,41004])

					if itemVnum in warrior_hair:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 1)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 5)

					if itemVnum in assassin_hair:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 4)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 0)

					if itemVnum in sura_hair:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 6)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 2)

					if itemVnum in shaman_hair:
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_MALE):
							self.__ModelPreview(itemVnum, 2, 3)
						if item.IsAntiFlag(item.ITEM_ANTIFLAG_FEMALE):
							self.__ModelPreview(itemVnum, 2, 7)
	...
	def __ModelPreview(self, Vnum, test, model):
		...
		self.ModelPreviewText.SetText("Prezentare")
	...
	def AppendPrice(self, price, isPrivateShop = False):	
		self.AppendSpace(5)
		if isPrivateShop:
			self.AppendTextLine(localeInfo.TOOLTIP_BUYPRICE  % (localeInfo.NumberToDragonCoinString(price)))
		else:
