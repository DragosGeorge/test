...
void CInputLogin::LoginByKey(LPDESC d, const char* data)
{
	...
	if (pinfo->NewCheckVersion != (int)LOGIN_VERSION)
	{
		TPacketGCLoginFailure failurePacket;
	...
	if (utils::is_empty(pinfo->warp_key))
	{
		d->SetPhase(PHASE_CLOSE);
		return;
	}
...
// #endif
	...
	if (ch->GetMapIndex() >= 10000)
	{
		...
		{
			// Need to improvement like Meley
...
#ifdef ENABLE_12ZI
	ch->BeadTime();
	if (ch->GetMapIndex() == MAP_12ZI_STAGE)
		ch->CheckFloor();
#endif
...
int CInputLogin::Analyze(LPDESC d, BYTE bHeader, const char* c_pData)
{
	...
	case HEADER_CG_WHISPER:
		if ((iExtraLen = Whisper(d, c_pData, m_iBufferLeft)) < 0)
			return -1;
		break;
